...
| 42 | Canvas | Hover ⓘ on a table opens table metadata | src/help/gestures.ts:13 | ☐ |
...
| 77 | Canvas | MiniMap is pannable and zoomable (lite colouring above the table threshold) | src/canvas/Canvas.tsx:604 | ☐ |
...
| 219 | API | POST `/api/export/png` writes a PNG from a base64 payload | src/api.ts:269 | ☐ |
...
| 243 | Editor | Collapse/expand the DBML editor pane | src/App.tsx:1696 | ☐ |
| 244 | Editor | Drag the vertical resizer to change editor width | src/App.tsx:1740 | ☐ |
...
| 259 | Editor | Autosave, when on, saves 1.5s after the document becomes dirty | src/App.tsx:425 | ☐ |
